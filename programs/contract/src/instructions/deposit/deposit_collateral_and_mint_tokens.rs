...
use crate::{
    deposit_sol, mint_tokens, Collateral, Config, SEED_COLLATERAL_ACCOUNT, SEED_CONFIG_ACCOUNT, SEED_SOL_ACCOUNT,check_health_factor
};
#[derive(Accounts)]
...
pub fn process_deposit_collateral_and_mint_tokens(
    ...
    mint_tokens(&ctx.accounts.mint_account,&ctx.accounts.token_program,&ctx.accounts.token_account,amount_to_mint,ctx.accounts.config_account.bump)?;
